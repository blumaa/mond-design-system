...
- 8987765: A MediaPlaceholder in a flex row stays where the row put it
...
- 575261c: ImageCarousel: a thumbnail is capped rather than given a share of the row.
...
- Updated dependencies [575261c]
  - @mond-design-system/tokens@4.2.0
...
- 6653ec4: Accessibility audit fixes across the library.
...
- fb05885: DataTable no longer folds into cards on a narrow screen.
...
- f80d603: Add the three anchored surfaces: `Popover`, `Menu` and `Tooltip`.
...
- Updated dependencies [f80d603]
  - @mond-design-system/tokens@4.1.0
...
- d3a6fe9: Drop the `ds-bridge` conformance CLI from this repo. It is no longer part of the
...
- Updated dependencies [d3a6fe9]
- Updated dependencies [d3a6fe9]
  - @mond-design-system/tokens@4.0.0
...
- 856b66b: Stack, Inline and Container keep what `as` promises: they take the props of the element they render, and they hand that element back through `ref`.
...
- 9c01984: A glyph now states its own size. An `svg` without one is sized by the browser,
...
- Updated dependencies [9c01984]
  - @mond-design-system/tokens@3.6.1
...
- Updated dependencies
  - @mond-design-system/tokens@3.6.0
...
- 0060582: `Skeleton` takes `lines`, standing in for a paragraph rather than a box.

  Above one line the last is cut short, where a paragraph's last line tends to
  land. A block of full-width bars reads as a table, not as prose — which is why
  every app that stacked skeletons by hand ended up re-deriving the same ragged
  edge. Text only: a rect or a circle is one shape however many lines of copy it
  replaces.
